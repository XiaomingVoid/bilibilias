...
object CCJsonToAss {
    ...
    fun formatSeconds(seconds: Double): String {
        ...
        val remainingSeconds = seconds - hours * 3600 - minutes * 60

        // 替代 DecimalFormat，手动格式化 00.00
        val intPart = remainingSeconds.toInt()
        val decPart = ((remainingSeconds - intPart) * 100).toInt().coerceIn(0, 99)
        val secsFormatted = buildString {
            append(intPart.toString().padStart(2, '0'))
            append('.')
            append(decPart.toString().padStart(2, '0'))
        }
        // 用你的跨平台 format
        return format("%d:%02d:%s", hours, minutes, secsFormatted)
    }
}