...
BILIBILIAS 当前是一个以 `:app` 为 Android 入口、`:shared` 承载跨端页面和平台桥接、多个 `core:*` module 提供基础能力的 Compose/Kotlin Multiplatform 项目。整体风格与 nowinandroid 一样偏向单 Activity、多 module、约定插件和声明式 UI，但本项目使用 Koin、Navigation 3、Room/DataStore/Ktor，并保留较多下载和平台接口相关逻辑。
...
跨端页面主要位于 `shared/src/commonMain/kotlin/com/imcys/bilibilias/shared/feature`，Android 入口和 Android-only 能力仍位于 `app`。公共 Compose 组件位于 `core/ui`，shared 业务组件位于 `shared/ui/component`。
...
已有页面大量使用 `collectAsStateWithLifecycle()` 收集 `StateFlow`。新增 UI 应优先复用 `core/ui/src/commonMain/kotlin/com/imcys/bilibilias/ui/component` 中的组件，例如 `ASTopAppBar`、`ASIconButton`、`ASTextButton`、`ASAlertDialog`、`ASAsyncImage`。
...
- `appModule` 注册 Android 应用层对象、下载管理器、App Functions、系统服务封装。
- `sharedModule` 注册 shared 页面 ViewModel、跨端运行时和 shared 层依赖。
...
下载相关代码当前分布在 `:app` 与 `:shared`。Android 侧下载执行、通知、FFmpeg 运行时和文件系统细节仍主要由 `:app` 承载；跨端页面可见的下载任务 model、命名规则、运行时抽象和部分格式转换工具位于 `:shared`。
...
- `shared.download.model`：下载任务、本地媒体资源、字幕和运行时信息。
- `shared.download.naming`：命名规则处理。
- `shared.download.runtime`：跨端下载执行器和运行时管理。
- `shared.download.util`：弹幕、字幕等格式转换工具。
